...
async function init() {
  ...
  const raw = await fetch('./lidar.csv').then(response => response.text())
  ...
  model = new THREE.Points(points, new THREE.PointsMaterial( { vertexColors: THREE.VertexColors, size: 0.25 }));
  scene.add(model);

...
async function init() {
  ...
  const raw = await fetch('./mesh.csv').then(response => response.text())
  //const raw = await fetch('./ground_roofs.csv').then(response => response.text())
  ...
  model = new THREE.Points(points, new THREE.PointsMaterial( { vertexColors: THREE.VertexColors, size: 0.15 }));
  scene.add(model);


  const sfm_points = new THREE.Geometry();
  const sfm = await fetch('./sfm.csv').then(response => response.text())
  sfm.split('\n').forEach( (line, i) => {
    const p = line.trim().split(',').map( j => parseFloat(j))
    if (!isNaN(p[0])) {
      ...
      const color = new THREE.Color();
      color.setRGB(p[3] / 255.0, p[4] / 255.0, p[5] / 255.0)
      sfm_points.vertices.push(new THREE.Vector3(p[0] - OFFSETX, p[1] - OFFSETY, p[2] - (OFFSETZ - 3.5)));
      sfm_points.colors.push(color);
    }
  })
  sfm_model = new THREE.Points(sfm_points, new THREE.PointsMaterial( { vertexColors: THREE.VertexColors, size: 0.25 }));
  scene.add(sfm_model);

  all_points = [model, sfm_model]
